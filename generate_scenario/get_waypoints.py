...
def get_position(sim, x, y):
    ...
    whole_position = sim.map_point_on_lane(position)
    position = whole_position.position
    # position.y = sim.map_point_on_lane(position).position.y
    rotation = whole_position.rotation
...
def get_vehicle_waypoints_time(sim, waypoints, idle=0):
    ...
    for i in range(len(waypoints)):
        ...
        if i == 0:
            ...
            # npc_waypoints.append(
            #     lgsvl.DriveWaypoint(transform.position, 0, angle=transform.rotation, idle=idle, deactivate=False))
        else:
            transform_former = get_position(sim, waypoints[i - 1][0], waypoints[i - 1][1])
            ...
            npc_waypoints.append(
                lgsvl.DriveWaypoint(transform.position, round(waypoints[i][2], 2), angle=rotation, idle=0, deactivate=False))
