...
from traffic_cases.get_critical_district import get_ego_next_lane
from generate_scenario.get_map import get_network
from generate_scenario.behavior_trajectory_solver.lane_filter import filter_lane
...
def sample_ego_init(network, road_type):
    ego_init = sample_road_points(network, road_type)
    while filter_lane(network, ego_init, road_type):
        sample_road_points(network, road_type)
...
def straight_road_init_point(network, position, road_type, same_lane=None, relative=None, reverse=False):
    road = network.findPointIn(Vector(position[0], position[1]), network.roads, False)
