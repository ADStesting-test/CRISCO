...
from generate_scenario.behavior_trajectory_solver.vehicle_cross import cross_init_position
...
def pedestrian_cross(network, road_type, ego_init, ego_dest, ego_speed, num, hazard_section):
    ...
    while rejection:
        ...
        # if road_type == 'urban':
        #     npc_init = get_npc_init_point(network, npc_dest, road_type, same_lane=False, reverse=True)
        ...
        if road_type == 'straight road':
            npc_init = straight_road_init_point(network, npc_dest, road_type, same_lane=False, reverse=False)
            npc_init_lane = network.laneAt(Vector(npc_init[0], npc_init[1]))
            npc_init_project_length = line_string.project(Point(npc_init[0], npc_init[1]))
            npc_dest_project_length = line_string.project(Point(npc_dest[0], npc_dest[1]))
        elif road_type == 'crossing' or road_type == 'T-junction':
            ...
            line_string = combine_center_line(network, npc_init_lane, npc_dest_lane, road_type)
            npc_init_project_length = line_string.project(Point(npc_init[0], npc_init[1]))
            npc_dest_project_length = line_string.project(Point(npc_dest[0], npc_dest[1]))
            interplot_lane = line_string
        ...
        for ans in ans_list:
            ...
            for i in range(int(c * 10)):
                ...
                p = interplot_lane.interpolate(pos)
                waypoints.append([p.x, p.y, speed])
            waypoint_sequence.append([waypoints, t-c+random.random()*2])
