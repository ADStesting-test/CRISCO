...
from traffic_cases.get_critical_district import get_hazard_section, get_roads


def vehicle_cross(network, road_type, ego_init, ego_dest, ego_speed, num, hazard_section):
    print("vehicle cross")
    ...
    while rejection:
        ...
        lane_string = combine_center_line(network, npc_init_lane, npc_dest_lane, road_type)
        ...
        for ans in ans_list:
            ...
            for i in range(int(c * 10)):
                ...
                p = lane_string.interpolate(pos)
                waypoints.append([p.x, p.y, speed])
            waypoint_sequence.append([waypoints, t-c+random.randint(1, 4)])

    return waypoint_sequence

def cross_init_position(network, road_type, npc_dest_road):
    district = get_roads(road_type)
    index = -2
    ...
    npc_init = straight_road_init_point(network, position, road_type)
