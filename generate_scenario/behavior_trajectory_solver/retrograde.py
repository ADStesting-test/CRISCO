...
def retrograde(network, road_type, ego_init, ego_dest, ego_speed, num, hazard_section):
    print("retrograde")
    ...
    while rejection:
        ...
        npc_init_project_length, npc_init_lane = retrograde_init_position(network, npc_dest, npc_dest_project_length, npc_dest_lane, road_type)
        ...
        position_c += [ego_initial_position == ego_init_project_length] + \
                    ...
                    [C * npc_average_speed == npc_initial_position - npc_target_position]
...
def retrograde_init_position(network, position, npc_dest_project_length, npc_dest_lane, road_type):
    npc_init = straight_road_init_point(network, position, road_type, relative='front', reverse=None)
    ...
    npc_dest_lane_length = npc_dest_lane.centerline.lineString.length
    while (npc_init_project_length - npc_dest_project_length) <= random.randint(30, 50) and npc_init_project_length != npc_dest_lane_length:
        npc_init = straight_road_init_point(network, position, road_type, relative='front', reverse=None)
        npc_point = Vector(npc_init[0], npc_init[1])
        npc_init_lane = network.laneAt(npc_point)
        npc_init_project_length = npc_dest_lane.centerline.lineString.project(Point(npc_init[0], npc_init[1]))
