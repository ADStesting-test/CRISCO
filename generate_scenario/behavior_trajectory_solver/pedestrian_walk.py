...
def pedestrian_walk(network, road_type, ego_init, ego_dest, ego_speed, num, hazard_section):
    print("pedestrian_walk")
    ...
    while rejection:
        ...
        npc_init = straight_road_init_point(network, npc_dest, road_type, same_lane=True, reverse=None)
        ...
        distance = abs(length)
        while distance < random.randint(5, 30):
            npc_init = straight_road_init_point(network, npc_dest, road_type, same_lane=True, reverse=None)
            npc_init_project_length = npc_dest_lane.centerline.lineString.project(Point(npc_init[0], npc_init[1]))
            length = npc_dest_project_length - npc_init_project_length
            distance = abs(length)
