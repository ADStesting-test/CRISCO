...
import lgsvl
from environment_generation.define_environment_svl import define_environment
...
from generate_scenario.determine_participants import cal_TTC
...
def test_generation(road_type):
    ...
    ego_destination, ego_spawn, ego_speed = define_ego_state(network, "middle", road_type)
    ...
    this_patterns, first_pattern, now_pattern = ["vehicle cross", "turn around"], [], []
    ...
    while new_ettc >= ettc:
        ...
        for behavior in first_pattern:
            trajectory = generate_behavior_trajectory(network, road_type, behavior, ego_spawn, ego_destination,
                                                      ego_speed, 5, "middle")
            new_ettc = cal_TTC(network, ego_spawn, ego_destination, ego_speed, trajectory, behavior, road_type)
        ...
        break
    selected, search = 0, 0
    while ettc > ettc_threshold and this_patterns is not None and search == selected:
        search = search + 1
                ...
                if behavior in now_pattern:
                    ...
                    for pattern_behavior in pattern:
                        trajectory = generate_behavior_trajectory(network, road_type, pattern_behavior, ego_spawn,
                                                                  ego_destination, ego_speed, 5, "middle")
                        new_ettc = cal_TTC(network, ego_spawn, ego_destination, ego_speed, trajectory, behavior, road_type)
...
def on_collision(agent1, agent2, contact):
    ...
    f.close()
    scenario_folder = os.path.join(root_path, "safety_violation_scenarios/" + road)
    file_nums = visitDir(scenario_folder)
    scenario_record = os.path.join(root_path,
                                   "safety_violation_scenarios/" + road + "/safety_violation" + str(file_nums))
...
def test_behavior(road_type, behavior):
    ...
    parameters = ["rainy", "foggy"]
    define_environment(sim, parameters)
    ...
    trajectory = generate_behavior_trajectory(network, road_type, behavior, ego_spawn, ego_destination, ego_speed, 5, "middle")
    if 'Pedestrian' in behavior:
        generate_pedestrian_participants(sim, trajectory)
    else:
        generate_vehicle_participants(sim, trajectory, behavior)
