...
def cal_TTC(network, init_ego, dst_ego, speed_ego, traj, behavior, road_type):
    ...
    ego_dest_project_length = line_string.project(Point(dst_ego[0], dst_ego[1]))
    if behavior == 'Park':
        park_length = line_string.project(Point(traj[0], traj[1]))
        ettc = random.choice([abs(park_length - ego_init_project_length) / speed_ego, abs(ego_dest_project_length - park_length) / speed_ego, 0])
    else:
        ettc = 200
        new_ettc = 200
